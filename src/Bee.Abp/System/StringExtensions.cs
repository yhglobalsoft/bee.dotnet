...
public static class StringExtensions
{
    ...
    public static string Substring(this string source, string startString, params string[] endStrings)
    {
        ...
        if (!string.IsNullOrEmpty(startString))
        {
            ...
            if (startIndex < 0)
            {
                throw new BusinessException(code: "Bee.Abp:0008").WithData("startString", startString);
            }
        ...
        if (endIndex < 0 || endIndex < startIndex)
        {
            throw new BusinessException(code: "Bee.Abp:0008").WithData("message", endStrings.ExpandAndToString());
        }
    ...
    /// 将JSON字符串转换为对象
    /// </summary> 
    public static T ToObj<T>(this string str, bool camelCase = false, bool indented = false, bool ignore = false)
    {
    ...
    /// <summary>
    /// 姓名脱敏
    /// 姓名两位最后一位为*,超过两位取第一位和最后一位中间为*
    /// </summary>
    public static string ToNameDesensitization(this string value)
    {
        ...
        var endValue = value.Substring(value.Length - 1, 1);
        if (value.Length > 2)
        {
            return startValue.PadRight(value.Length - 1, '*') + endValue;
        }
    ...
    /// 电话的第4-7位显示为*
    /// </summary>
    public static string ToPhoneNumberDesensitization(this string value)
    {
        if (value.IsNullOrWhiteSpace())
        {
            return null;
        ...
        switch (value.Length)
        {
            case <= 3:
                return value;
            case > 3 and <= 7:
                return value.Substring(0, 3).PadRight(value.Length, '*');
            case > 7:
            {
